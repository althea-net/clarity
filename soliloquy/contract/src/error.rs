...
pub trait EthError: Tokenizable + AbiDecode + AbiEncode + Send + Sync {
    ...
    fn from_rpc_response(response: &Web3Error) -> Option<Self> {
...
mod test {
    ...
    use soliloquy_core::types::Bytes;
    ...
    fn string_error() {
        ...
        assert_eq!(
            String::selector().as_slice(),
            &multicall_revert_string[0..4]
        );
        assert_eq!(
            String::decode_with_selector(&multicall_revert_string)
                .unwrap()
                .as_str(),
    ...
    fn custom_error() {
        ...
        assert!(
            ExampleContractErrors::valid_selector(selector),
            "selector is valid"
        );
    ...
    fn custom_error_string() {
        ...
        assert_eq!(
            e,
            ExampleContractErrors::RevertString("Multicall3: call failed".into())
        );
    }
