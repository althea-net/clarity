...
pub(crate) fn impl_derive_eip712(input: &DeriveInput) -> Result<TokenStream> {
    ...
    // Use reference to soliloquy_core instead of directly using the crate itself.
    let soliloquy_core = soliloquy_core_crate();

    let tokens = quote! {
        impl #soliloquy_core::types::transaction::eip712::Eip712 for #primary_type {
            type Error = #soliloquy_core::types::transaction::eip712::Eip712Error;
            ...
            fn domain(&self) -> ::core::result::Result<#soliloquy_core::types::transaction::eip712::EIP712Domain, Self::Error> {
                #soliloquy_core::utils::__serde_json::from_str(#domain_str).map_err(::core::convert::Into::into)
            }

            fn struct_hash(&self) -> ::core::result::Result<[u8; 32], Self::Error> {
                let mut items = vec![#soliloquy_core::abi::Token::Uint(
                    #soliloquy_core::types::U256::from(&Self::type_hash()?[..]),
                )];

                if let #soliloquy_core::abi::Token::Tuple(tokens) =
                    #soliloquy_core::abi::Tokenizable::into_token(::core::clone::Clone::clone(self))
                {
                    items.reserve(tokens.len());
                    for token in tokens {
                        match &token {
                            #soliloquy_core::abi::Token::Tuple(t) => {
                            ...
                            _ => {
                                items.push(#soliloquy_core::types::transaction::eip712::encode_eip712_type(token));
                            }
                        }
                    }
                }

                let struct_hash = #soliloquy_core::utils::keccak256(#soliloquy_core::abi::encode(
                    &items,
                ));
