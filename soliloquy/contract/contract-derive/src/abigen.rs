...
use contract_abigen::{multi::MultiExpansion, Abigen};
...
impl Parse for Contracts {
    fn parse(input: ParseStream) -> Result<Self> {
        let inner = input
            .parse_terminated(ContractArgs::parse, Token![;])?
            .into_iter()
            .collect();
        Ok(Self { inner })
    }
...
impl ContractArgs {
    fn into_builder(self) -> Result<Abigen> {
        ...
        for parameter in self.parameters {
            match parameter {
                Parameter::Methods(methods) => builder.method_aliases_mut().extend(
                    methods
                        .into_iter()
                        .map(|m| (m.signature, m.alias.to_string())),
                ),
...
impl Parse for Method {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        for (i, param) in params.into_iter().enumerate() {
            ...
            // validate
            soliloquy_core::abi::ethabi::param_type::Reader::read(&s)
...
mod tests {
    ...
    fn derives<'a>(v: impl IntoIterator<Item = &'a str>, trailing: bool) -> Parameter {
        let mut derives: Punctuated<_, _> = v
            .into_iter()
