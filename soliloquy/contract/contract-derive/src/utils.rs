...
use soliloquy_core::{abi::ParamType, macros::soliloquy_core_crate, types::Selector};
use syn::{
...
pub fn signature(hash: &[u8]) -> TokenStream {
    ...
    quote! {#soliloquy_core::types::H256([#( #bytes ),*])}
}
...
pub fn topic_param_type_quote(kind: &ParamType) -> TokenStream {
    ...
    match kind {
        ...
        | ParamType::Tuple(_) => quote! {#soliloquy_core::abi::ParamType::FixedBytes(32)},
...
pub fn param_type_quote(kind: &ParamType) -> TokenStream {
    ...
    match kind {
        ParamType::Address => {
            quote! {#soliloquy_core::abi::ParamType::Address}
        }
        ParamType::Bytes => {
            quote! {#soliloquy_core::abi::ParamType::Bytes}
        }
        ParamType::Int(size) => {
            let size = Literal::usize_suffixed(*size);
            quote! {#soliloquy_core::abi::ParamType::Int(#size)}
        }
        ParamType::Uint(size) => {
            let size = Literal::usize_suffixed(*size);
            quote! {#soliloquy_core::abi::ParamType::Uint(#size)}
        }
        ParamType::Bool => {
            quote! {#soliloquy_core::abi::ParamType::Bool}
        }
        ParamType::String => {
            quote! {#soliloquy_core::abi::ParamType::String}
        }
        ParamType::Array(ty) => {
            let ty = param_type_quote(ty);
            quote! {#soliloquy_core::abi::ParamType::Array(Box::new(#ty))}
        }
        ParamType::FixedBytes(size) => {
            let size = Literal::usize_suffixed(*size);
            quote! {#soliloquy_core::abi::ParamType::FixedBytes(#size)}
        }
        ParamType::FixedArray(ty, size) => {
            let ty = param_type_quote(ty);
            let size = Literal::usize_suffixed(*size);
            quote! {#soliloquy_core::abi::ParamType::FixedArray(Box::new(#ty), #size)}
        }
        ParamType::Tuple(tuple) => {
            let elements = tuple.iter().map(param_type_quote);
            quote!(#soliloquy_core::abi::ParamType::Tuple(::std::vec![#( #elements ),*]))
        }
    }
}
...
pub fn derive_param_type_with_abi_type(
    ...
    Ok(quote! {
        #soliloquy_core::abi::ParamType::Tuple(::std::vec!#params)
    })
}
...
pub fn abi_parameters_array(input: &DeriveInput, trait_name: &str) -> Result<TokenStream, Error> {
    ...
    let iter = fields.iter().map(|f| {
        let ty = &f.ty;
        quote_spanned!(f.span() => <#ty as #soliloquy_core::abi::AbiType>::param_type())
    });
...
mod tests {
    ...
    fn can_find_params() {
        ...
        let test_cases: &[(Type, ParamType)] = type_test_cases! {
            ...
            ethers::types::U256 => PT::Uint(256),
            ::soliloquy_core::types::U256 => PT::Uint(256),
            soliloquy_core::types::U256 => PT::Uint(256),
            U256 => PT::Uint(256),
            ...
            ethers::types::I256 => PT::Int(256),
            ::soliloquy_core::types::I256 => PT::Int(256),
            soliloquy_core::types::I256 => PT::Int(256),
            I256 => PT::Int(256),
            ...
            ::std::vec::Vec<::soliloquy_core::types::U256, ::std::alloc::Global> => arr(PT::Uint(256)),
            ::std::vec::Vec<::soliloquy_core::types::U256, Global> => arr(PT::Uint(256)),
            ::std::vec::Vec<::soliloquy_core::types::U256> => arr(PT::Uint(256)),
