...
use contract_abigen::Source;
use proc_macro2::{Span, TokenStream};
use quote::quote;
use soliloquy_core::{
    abi::{Event, EventExt, EventParam, HumanReadableParser},
    macros::{soliloquy_contract_crate, soliloquy_core_crate},
};
...
pub(crate) fn derive_eth_event_impl(input: DeriveInput) -> Result<TokenStream> {
    ...
    let ethers_contract = soliloquy_contract_crate();
    ...
    let ethevent_impl = quote! {
        impl #ethers_contract::EthEvent for #name {
            ...
            fn signature() -> #soliloquy_core::types::H256 {
                #signature
            }
            ...
            fn decode_log(log: &#soliloquy_core::abi::RawLog) -> ::core::result::Result<Self, #soliloquy_core::abi::Error> where Self: Sized {
                #decode_log_impl
            }
...
fn derive_decode_from_log_impl(input: &DeriveInput, event: &Event) -> Result<TokenStream> {
    ...
    let (signature_check, flat_topics_init, topic_tokens_len_check) = if event.anonymous {
        (
            ...
            quote! {
                if topic_tokens.len() != topics.len() {
                    return Err(#soliloquy_core::abi::Error::InvalidData);
                }
            },
        )
    } else {
        (
            Some(quote! {
                let event_signature = topics.get(0).ok_or(#soliloquy_core::abi::Error::InvalidData)?;
    ...
    if fields.is_empty() {
        return Ok(quote! {
            ...
            let #soliloquy_core::abi::RawLog {topics, data} = log;
            ...
            #soliloquy_core::abi::Tokenizable::from_token(#soliloquy_core::abi::Token::Tuple(::std::vec::Vec::new())).map_err(|_|#soliloquy_core::abi::Error::InvalidData)
        });
    }
    ...
    {
        quote! {
            let topic_tokens = #soliloquy_core::abi::decode(&topic_types, &flat_topics)?;
            #topic_tokens_len_check
            let data_tokens = #soliloquy_core::abi::decode(&data_types, data)?;
        ...
        quote! {
            let mut topic_tokens = #soliloquy_core::abi::decode(&topic_types, &flat_topics)?;
            #topic_tokens_len_check
            let mut data_tokens = #soliloquy_core::abi::decode(&data_types, &data)?;
    ...
    Ok(quote! {
        ...
        let #soliloquy_core::abi::RawLog {data, topics} = log;
        ...
        #soliloquy_core::abi::Tokenizable::from_token(#soliloquy_core::abi::Token::Tuple(tokens)).map_err(|_|#soliloquy_core::abi::Error::InvalidData)
    })
}
