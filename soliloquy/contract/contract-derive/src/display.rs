...
use quote::quote;
use soliloquy_core::{abi::ParamType, macros::soliloquy_core_crate};
...
pub(crate) fn derive_eth_display_impl(input: DeriveInput) -> Result<TokenStream, Error> {
    let fields = match input.data {
        ...
        Data::Enum(e) => {
            return Err(Error::new(
                e.enum_token.span(),
                "Enums are not supported by EthDisplay",
            ))
        }
        Data::Union(u) => {
            return Err(Error::new(
                u.union_token.span(),
                "Unions are not supported by EthDisplay",
            ))
        }
    };
    ...
    for (i, field) in fields.enumerate() {
        ...
        if let Ok(param) = utils::find_parameter_type(&field.ty) {
            ...
            let hex_encode = quote!(#soliloquy_core::utils::hex::encode);
