...
pub(crate) fn generate<T: Verbatim>(t: &T, soliloquy_core: &Path) -> TokenStream {
    let mut s = TokenStream::new();
    t.to_tokens(&mut s, soliloquy_core);
    s
}

/// Generates the constructor tokens.
pub(crate) trait Verbatim {
    fn to_tokens(&self, s: &mut TokenStream, soliloquy_core: &Path);
}
...
impl<T: ?Sized + Verbatim> Verbatim for Box<T> {
    fn to_tokens(&self, s: &mut TokenStream, soliloquy_core: &Path) {
        ...
        (**self).to_tokens(&mut t, soliloquy_core);
...
impl<T: Verbatim> Verbatim for Vec<T> {
    fn to_tokens(&self, s: &mut TokenStream, soliloquy_core: &Path) {
        let iter = self.iter().map(|x| ToTokensCompat(x, soliloquy_core));
...
impl<K: Verbatim, V: Verbatim> Verbatim for BTreeMap<K, V> {
    fn to_tokens(&self, s: &mut TokenStream, soliloquy_core: &Path) {
        ...
        let iter = self.iter().map(|(k, v)| {
            let mut s = TokenStream::new();
            k.to_tokens(&mut s, soliloquy_core);
            s.append(Punct::new(',', proc_macro2::Spacing::Alone));
            v.to_tokens(&mut s, soliloquy_core);
            quote! { (#s), }
        });
...
macro_rules! derive_verbatim {
    ...
    (struct $name:ident { $($field:ident),* $(,)? } $($rest:tt)*) => {
        impl Verbatim for soliloquy_core::abi::ethabi::$name {
            #[allow(deprecated)]
            fn to_tokens(&self, s: &mut TokenStream, soliloquy_core: &Path) {
                $(
                    let $field = ToTokensCompat(&self.$field, soliloquy_core);
                )*
                s.extend(quote! {
                    #soliloquy_core::abi::ethabi::$name {
                        $($field: #$field,)*
                    }
                });
            }
        ...
        impl Verbatim for soliloquy_core::abi::ethabi::$name {
            ...
            fn to_tokens(&self, s: &mut TokenStream, soliloquy_core: &Path) {
                match self {$(
                    Self::$variant $(($($field),*))? => {
                        $($(
                            let $field = ToTokensCompat($field, soliloquy_core);
                        )*)?
                        s.extend(quote! {
                            #soliloquy_core::abi::ethabi::$name::$variant $(($(#$field),*))?
                        });
                    }
                )*}
            }
