...
impl Context {
    ...
    pub fn from_abigen(args: Abigen) -> Result<Self> {
        ...
        let (abi, human_readable, abi_parser) = parse_abi(&abi_str).map_err(|e| {
            eyre::eyre!(
                "error parsing abi for contract '{}': {e}",
                args.contract_name
            )
        })?;
    ...
    pub(crate) fn struct_declaration(&self) -> TokenStream {
        ...
        let abi = {
            ...
            let abi = crate::verbatim::generate(&self.abi, &soliloquy_core);
            quote! {
                #[allow(deprecated)]
                fn __abi() -> #soliloquy_core::abi::Abi {
                    #abi
                }

                #[doc = #doc_str]
                pub static #abi_name: #ethers_contract::Lazy<#soliloquy_core::abi::Abi> =
        ...
        let bytecode = self.contract_bytecode.as_ref().map(|bytecode| {
            ...
            quote! {
                ...
                pub static #bytecode_name: #soliloquy_core::types::Bytes =
                    #soliloquy_core::types::Bytes::from_static(__BYTECODE);
            }
        });
        ...
        let deployed_bytecode = self.contract_deployed_bytecode.as_ref().map(|bytecode| {
            ...
            quote! {
                ...
                pub static #bytecode_name: #soliloquy_core::types::Bytes =
                    #soliloquy_core::types::Bytes::from_static(__DEPLOYED_BYTECODE);
            }
        });
