...
impl SolInput {
    ...
    pub fn normalize_json(self) -> Result<Self> {
        ...
        let sol_interface_tokens: TokenStream = std::iter::once(TokenTree::Ident(id("interface")))
            .chain(all_tokens)
            .collect();
...
```",
        ...
        let ast: syn_solidity::File = syn::parse2(tokens).map_err(|e| {
            let msg = format!(
...
mod tests {
    ...
    fn parse_test(s: &str, path: &str) {
        ...
        let _ast = match syn::parse2::<syn_solidity::File>(tokens.clone()) {
            Ok(ast) => ast,
            Err(e) => {
