...
impl<T, P: fmt::Debug, E, N> fmt::Debug for Event<T, P, E, N> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Event")
            ...
            .field(
                "event_type",
                &format_args!("{}", std::any::type_name::<E>()),
            )
            .finish()
    }
}
...
impl<T, P: Provider<N>, E: SolEvent, N: Network> Event<T, P, E, N> {
    ...
    pub fn is_pending_block_filter(&self) -> bool {
        self.filter
            .block_option
            .get_from_block()
            .is_some_and(BlockNumberOrTag::is_pending)
            && self
                .filter
                .block_option
                .get_to_block()
...
mod tests {
    ...
    async fn event_filters() {
        ...
        ); // add check that the received event signature is the same as the one we expect
