...
/// `ethers_crate => name`
type CrateNames = HashMap<SoliloquyCrate, &'static str>;
...
static ETHERS_CRATE_NAMES: Lazy<CrateNames> = Lazy::new(|| {
    ProjectEnvironment::new_from_env()
        ...
        .unwrap_or_else(|| SoliloquyCrate::ethers_path_names().collect())
});

/// Returns the `core` crate's [`Path`][syn::Path].
#[inline]
pub fn soliloquy_core_crate() -> syn::Path {
    get_crate_path(SoliloquyCrate::SoliloquyCore)
}

/// Returns the `contract` crate's [`Path`][syn::Path].
#[inline]
pub fn soliloquy_contract_crate() -> syn::Path {
    get_crate_path(SoliloquyCrate::SoliloquyContract)
}

/// Returns an [`EthersCrate`]'s [`Path`][syn::Path] in the current project.
#[inline(always)]
pub fn get_crate_path(krate: SoliloquyCrate) -> syn::Path {
    krate.get_path()
}
...
impl ProjectEnvironment {
    ...
    fn crate_names_from_metadata(&self) -> Option<CrateNames> {
        ...
        // available
        if pkg.name.parse::<SoliloquyCrate>().is_ok() || pkg.name == "ethers" {
            return Some(SoliloquyCrate::path_names().collect());
        }

        let mut names: CrateNames = SoliloquyCrate::ethers_path_names().collect();
        for dep in pkg.dependencies.iter() {
            let name = dep.name.as_str();
            if name.starts_with("ethers") {
                if name == "ethers" {
                    return None;
                } else if let Ok(dep) = name.parse::<SoliloquyCrate>() {
...
pub enum SoliloquyCrate {
    SoliloquyContract,
    SoliloquyContractAbigen,
    SoliloquyContractDerive,
    SoliloquyCore,
}

impl AsRef<str> for SoliloquyCrate {
    fn as_ref(&self) -> &str {
        self.crate_name()
    }
}

impl fmt::Display for SoliloquyCrate {
...
impl SoliloquyCrate {
    ...
    pub const fn crate_name(self) -> &'static str {
        match self {
            Self::SoliloquyContract => "contract",
            Self::SoliloquyContractAbigen => "contract-abigen",
            Self::SoliloquyContractDerive => "contract-derive",
            Self::SoliloquyCore => "soliloquy-core",
        }
    }
    ...
    pub const fn path_name(self) -> &'static str {
        match self {
            Self::SoliloquyContract => "::contract",
            Self::SoliloquyContractAbigen => "::contract_abigen",
            Self::SoliloquyContractDerive => "::contract_derive",
            Self::SoliloquyCore => "::soliloquy_core",
        }
    }
    ...
    pub const fn ethers_path_name(self) -> &'static str {
        match self {
            ...
            Self::SoliloquyContractAbigen => "::contract", // partially
            Self::SoliloquyContractDerive => "::contract",
    ...
    pub const fn fs_path(self) -> &'static str {
        match self {
            Self::SoliloquyContractAbigen => "contract/contract-abigen",
            Self::SoliloquyContractDerive => "contract/contract-derive",
            _ => self.crate_name(),
        }
    }
...
mod tests {
    ...
    impl Distribution<SoliloquyCrate> for Standard {
        fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> SoliloquyCrate {
            const RANGE: std::ops::Range<u8> = 0..SoliloquyCrate::COUNT as u8;
    ...
    fn test_names() {
        fn assert_names(s: &ProjectEnvironment, ethers: bool, dependencies: &[SoliloquyCrate]) {
            ...
            let krate = s
                .crate_name
                .as_ref()
                .and_then(|x| x.parse::<SoliloquyCrate>().ok());
            let is_internal = krate.is_some();
            let expected: CrateNames = match (is_internal, ethers) {
                // internal
                (true, _) => SoliloquyCrate::path_names().collect(),

                // ethers
                (_, true) => SoliloquyCrate::ethers_path_names().collect(),

                // no ethers
                (_, false) => {
                    let mut n: CrateNames = SoliloquyCrate::ethers_path_names().collect();
        ...
        fn gen_unique<const N: usize>() -> [SoliloquyCrate; N] {
            assert!(N < SoliloquyCrate::COUNT);
    ...
    fn write_manifest(s: &ProjectEnvironment, ethers: bool, dependencies: &[SoliloquyCrate]) {
